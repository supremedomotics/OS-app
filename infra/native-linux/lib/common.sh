...
# SUPREME_SECRETS_DIR. That directory is deliberately root:${SUPREME_GROUP} mode 0700 —
# every other secret there is written ONCE by root, and the gateway only ever reads what
# root already provisioned (§ established security model) — so the gateway's own write
# attempt hit a real EACCES on every boot, live-confirmed on a real deployment. Minted
# here instead, the same way as every other secret, root-owned and group-readable.
#
