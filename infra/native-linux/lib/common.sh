...
# § live-confirmed fix — the driver-secret encryption key (AES-256-GCM, used to encrypt
# every installed driver's config secrets at rest — see @supreme/crypto's
# generateEncryptionKey/encryptSecret) used to be lazily generated by the UNPRIVILEGED
# supreme-gateway process itself on first boot, via a plain filesystem write into
# SUPREME_SECRETS_DIR — hit a real EACCES on every boot, live-confirmed on a real
# deployment, because that directory is root:${SUPREME_GROUP}. Minted here instead, the
# same way as every other secret, root-owned and group-readable.
#
# § live-confirmed fix #2 — minting the FILE as 0640 group-readable wasn't enough on its
# own: SUPREME_SECRETS_DIR itself was 0700 (owner-only, zero group access), which blocks
# the supreme group from ever traversing INTO the directory regardless of what mode the
# files inside carry — readFileSync fails as if the file didn't exist at all, so the
# gateway fell through to the write path and hit the exact same EACCES again. This was
# always latent (every other secret there is read only by root-run scripts, never by the
# unprivileged runtime process) but never surfaced until this key needed a runtime read.
# The chmod 0750 below is deliberately repeated on every call (not just at creation) so
# it self-heals any already-deployed box still carrying the old 0700 mode, without
# requiring a manual recover.sh run.
#
# Called from BOTH install.sh's persist_secrets() (fresh installs) and load_secrets()
# below (routine update.sh runs on an already-installed box, which never calls
# persist_secrets again) — an existing deployment that predates this fix needs the file
# created too, without waiting for a from-scratch install.sh re-run.
#
# CRITICAL: generate-once, NEVER regenerate once the file exists — unlike token-secret/
# postgres-password (safe to reuse the same value every run), changing this key would
# make every ALREADY-encrypted driver secret in the database permanently undecryptable.
# Base64, not hex (generate_secret's own format) — the crypto library requires exactly
# 32 raw bytes, decoded via `Buffer.from(key, "base64")`.
ensure_driver_secret_encryption_key() {
  local key_file="${SUPREME_SECRETS_DIR}/driver_secret_encryption_key"
  mkdir -p "$SUPREME_SECRETS_DIR"
  chmod 0750 "$SUPREME_SECRETS_DIR"
  chown "root:${SUPREME_GROUP}" "$SUPREME_SECRETS_DIR"
  [ -s "$key_file" ] && return
  log_info "Minting the driver-secret encryption key (first run only — never regenerated afterward)"
  (
    umask 077
    head -c 32 /dev/urandom | base64 > "$key_file"
  )
  chown "root:${SUPREME_GROUP}" "$key_file"
  chmod 0640 "$key_file"
}
