...
| `GET /devices/{address}/now_playing` | current app + content + transport |
...
which is enough for **unicast control by IP** once you know the address. For reliable
**discovery** from inside Docker, run the bridge with host networking
(`network_mode: host`) or pass the address to `/pair/begin` directly (control works
without discovery).
