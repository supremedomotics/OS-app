...
/// A glyph-friendly chip — an icon kind + its count label ("3", "100%").
class RoomChip {
  const RoomChip(this.kind, this.label);
  final String kind; // light | climate | media | fan | switch | cover
  final String label;
}

/// Structured, glyph-first room summary — luxury communicates before you read.
List<RoomChip> roomChips(List<Device> devices) {
  ...
  for (final d in devices) {
    ...
    if (caps.contains('temperature')) { final m = s('temperature')?['mode']; if (m != null && m != 'off') climate++; continue; }
    if (caps.contains('media')) { final pb = s('media')?['playback']; if (pb == 'playing' || pb == 'paused') media++; continue; }
    if (caps.contains('fan')) { if (s('fan')?['on'] == true) fans++; continue; }
    if (caps.contains('onoff')) { if (s('onoff')?['on'] == true) switches++; continue; }
  }
  final chips = <RoomChip>[];
  if (lights > 0) chips.add(RoomChip('light', '$lights'));
  if (climate > 0) chips.add(RoomChip('climate', '$climate'));
  if (media > 0) chips.add(RoomChip('media', '$media'));
  if (fans > 0) chips.add(RoomChip('fan', '$fans'));
  if (switches > 0) chips.add(RoomChip('switch', '$switches'));
  if (coversOpen.length == 1) {
    chips.add(RoomChip('cover', '${coversOpen.first}%'));
  } else if (coversOpen.length > 1) {
    chips.add(RoomChip('cover', '${coversOpen.length}'));
  }
  return chips;
}
